...
SEXP AddNode(SEXP net_ptr, SEXP node_name, SEXP node_type, SEXP node_states) {
  ...
  if( node_type_s == "chance" ){
    ...
    Rcout<< "Unknown node type:" << node_type_s << "!" <<endl;
    Rcout<< "Exiting!!!!" <<endl; //PUT IN PROPER ERROR HANDELING FOR PRODUCTION!
    return wrap(-1);
  }
  //Check to see if the dsl_node_typ is still negative. Indicates there still is a problem.
  if(dsl_node_typ < 0){
    Rcout<< "There was a problem. Execution for pointers out of order??" <<endl;
    Rcout<< "Exiting!!!!" <<endl; //PUT IN PROPER ERROR HANDELING FOR PRODUCTION!
    return wrap(-1);
  }
  
  //Rcout<< dsl_node_typ << endl;
